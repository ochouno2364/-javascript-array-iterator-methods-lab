...
let veryOldInventors = inventors.filter(inventors => inventors.year <= 1599 && inventors.year >= 1500);
...
let inventorNamedAda = inventors.find(findAda);

// Complete the exercise in the space below:
function findAda(inventors) {
...
// Complete the exercise in the space below:
firstLast = people.map((firstLastName)=> {
    const peopleName = firstLastName.split(', ')
    return `${peopleName[1]} ${peopleName[0]}`
    })
...
// Complete the exercise in the space below:
isEveryone19OrOlder = devs.every((dev)=> dev.year <= 2005)
// Check your work:
